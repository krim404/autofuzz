...
# Initiale Version von F-Secure (mqtt_fuzz)
#
...
class GeneralFuzzProtocol(Protocol):
...
class GeneralClientFactory(ClientFactory):
    '''Factory that creates pseudo clients'''

    protocol = GeneralFuzzProtocol

    def __init__(self, fuzz_proto, fuzz_ratio, send_delay, radamsa_path, validcases_path):
        # Copy the data into this instance so we can use it later
        self.fuzz_proto = fuzz_proto
        ...
        # Read configuration file from valid cases
        with open(self.validcases_path + self.fuzz_proto + "/structures.csv", 'r') as f:
            for line in f:
                line = line.rstrip()

                if len(line) != 0:
                    elem = line.split(',')
                    self.session_structures.append(elem)
    ...
    def buildProtocol(self, address):
        ...
        protocol_instance.validcases_path = self.validcases_path + self.fuzz_proto + "/"
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        print("%s:Failed to connect to server: %s" % (calendar.timegm(time.gmtime()), reason))
        reactor.stop()
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        # instance)
        print("%s:Connection to server lost: %s" % (calendar.timegm(time.gmtime()), reason))
        print("%s:Reconnecting" % calendar.timegm(time.gmtime()))
        connector.connect()

def run_tests(proto, host, port, ratio, delay, radamsa, validcases):  # pylint: disable=R0913
    '''Main function to run'''
    from twisted.internet import reactor

    factory = GeneralClientFactory(proto, ratio, delay, radamsa, validcases)
    hostname = host
    port = int(port)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='fuzz, a simple protocol fuzzer.')
    parser.add_argument('protocol', metavar='target_protocol',
                        type=str,
                        default='mqtt',
                        help='Used protocol, default mqtt')
    parser.add_argument('host', metavar='target_host',
                        type=str,
                        default='localhost',
                        help='Host name of server / broker under test')
    parser.add_argument('port', metavar='target_port',
                        type=int,
                        default=1883,
                        help='Port number of server / broker under test')
    ...
    parser.add_argument('-validcases', metavar='validcase_path',
                        ...
                        default='valid/', help='Path to the valid-case directories, default is "valid-cases/"')
    ...
    run_tests(args.protocol, args.host, args.port, args.ratio, args.delay, args.fuzzer, args.validcases)
